...
TEST(b_tests, file_creation_one) {
    ...
    const char *test_fname = "b_tests_normal.f16fs";
...
TEST(b_tests, file_creation_two) {
    ...
    const char *test_fname = "b_tests_full_table.f16fs";
    F16FS_t *fs            = fs_format(test_fname);
...
TEST(c_tests, open_close_file) {
    ...
    const char *test_fname = "c_tests.f16fs";

    ASSERT_EQ(system("cp b_tests_normal.f16fs c_tests.f16fs"), 0);
...
TEST(d_tests, write_file_simple) {
    ...
    const char *test_fname = "d_tests_normal.f16fs";
...
TEST(d_tests, write_file_fill) {
    ...
    const char *test_fname = "d_tests_full.f16fs";
    ...
    // According to my investigation, there's 201 blocks left
    ASSERT_EQ(fs_write(fs, fd, giant_data_hunk, 512 * 256), 512 * 201);
...
TEST(e_tests, remove_file) {
    ...
    const char *(test_fname[2]) = {"e_tests_a.f16fs", "e_tests_b.f16fs"};
...
TEST(g_tests, seek) {
    ...
    const char *test_fname = "g_tests.f16fs";

    ASSERT_EQ(system("cp d_tests_full.f16fs g_tests.f16fs"), 0);
...
TEST(h_tests, read) {
    ...
    // FS_READ 11
    ASSERT_EQ(fs_seek(fs, fd, 98675309, FS_SEEK_CUR), 33398272);
    ASSERT_EQ(fs_seek(fs, fd, -500, FS_SEEK_END), 33397772);
    nbyte = fs_read(fs, fd, write_space, 1024);
    ASSERT_EQ(nbyte, 500);
    ASSERT_EQ(memcmp(write_space, six_e, 500), 0);
    // did you mess up the position?
    ASSERT_EQ(fs_seek(fs, fd, 0, FS_SEEK_CUR), 33398272);
...
TEST(i_tests, move) {
    ...
    ASSERT_EQ(system("cp c_tests.f16fs g_tests.f16fs"), 0);
...
/*
    These are yours to implement. I suggest you base it off of the tests for move as they're similar
